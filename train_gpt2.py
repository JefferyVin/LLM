...
class GPT(nn.Module):
    ...
    def configure_optimizers(self, weight_decay, learning_rate, device):
        ...
        print(f"number of parameters: {num_decay_params} (decayed), {num_nodecay_params} (not decayed)")
        fused_available = 'fused' in inspect.signature(torch.optim.AdamW).parameters
        use_fused = fused_available and 'cuda' in device
        print(f"using fused adamw: {use_fused}")
        optimizer = torch.optim.AdamW(optim_groups, lr=learning_rate, betas=(0.9, 0.95), eps=1e-8, amsgrad=False)
        return optimizer
...
for step in range(max_steps):
    ...
    print(f"step {step:4d} | loss: {loss_accum.item():.6f} | lr: {lr:.4e} | norm: {norm:.4f} | tokens/sec: {tokens_per_sec} | time: {dt*1000} ms")